...
CREATE TABLE example_schema.users (
    ...
    geom GEOMETRY(Point, 4326),          -- Geometry column of type Point with SRID 4326 (Example)
    element VARCHAR(100) NOT NULL,       -- Element name, not null
    physical_conservation VARCHAR(100) UNIQUE NOT NULL, -- Unique and not null physical conservation (Example)
    registration_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP -- Registration date, default to the current time (Example)
);  --and so on
